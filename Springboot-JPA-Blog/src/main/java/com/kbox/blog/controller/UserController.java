...
import com.kbox.blog.model.KakaoProfile;
...
public class UserController {
	...
	public String kakaoCallback(String code) { // Data를 리턴해주는 컨트롤러 함수
		...
		params.add("client_id", "e427e7cc129f9d335bcb52983fbbfd9c");
		...
		System.out.println("블로그서버 패스워드 : " + kboxKey);

		User KakaoUser = User.builder().username(kakaoProfile.getKakao_account().getEmail() + "_" + kakaoProfile.getId())
				.password(kboxKey).email(kakaoProfile.getKakao_account().getEmail()).oauth("kakao").build();

		// 가입자 혹은 비가입자 체크 해서 처리
		User originUser = userService.회원찾기(KakaoUser.getUsername());

		if (originUser.getUsername() == null) {
			System.out.println("기존 회원이 아닙입니다...!!");
			userService.회원가입(KakaoUser);
		}

		System.out.println("자동 로그인 진행합니다.");
		// 로그인 처리
		Authentication authentication = authenticationManager.authenticate(new UsernamePasswordAuthenticationToken(KakaoUser.getUsername(), kboxKey));
